...
class SensorSelection_Env(gym.Env):
    def __init__(self, config_file=""):
        ...
        self.env = ENV.Environment(
            self.terrain, time_of_day=12, thermal=self.thermal_manager
        )
        ...
        self.blue_team = TEAM.Team(
            team_name="blue",
            config=self.config["blue_agent"],
            thermal=self.thermal_manager,
        )
        self.red_team = TEAM.Team(
            team_name="red",
            config=self.config["red_agent"],
            thermal=self.thermal_manager,
        )
        ...
        self.action_space = spaces.MultiDiscrete(
            [sensor_combinations] * self.blue_team.Num_agents
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        for agent in self.blue_team.agents:
            agent._reset_states(
                terrain_bound=self.env.terrain["terrain_bounds"][0, :],
                physicsClient=self.physics_client,
                team=self.blue_team.team_color,
            )
        ...
        p.resetDebugVisualizerCamera(
            ...
            cameraTargetPosition=tuple(self.blue_team.agents[0].position.T.tolist()[0]),
        )
        if (
            self.camera_viewer["blue"].isVisible()
            and self.camera_viewer["red"].isVisible()
        ):
    ...
    def _get_observation(self):
        pos, _ = p.getBasePositionAndOrientation(
            self.blue_team.agents[0].agent_id, physicsClientId=self.physics_client
        )
    ...
    def step(self, action):
        ...
        # Move agent
        p.resetBaseVelocity(
            self.blue_team.agents[0].agent_id,
            linearVelocity=[dx, dy, 0],
        ...
        p.resetDebugVisualizerCamera(
            ...
            cameraYaw=self.camera_yaw,
            cameraPitch=self.camera_pitch,
            cameraTargetPosition=blue_states[0]["pos"],
        )
        ...
        if (
            ...
            for idx, agent in enumerate(self.blue_team.agents):
                pos, _ = p.getBasePositionAndOrientation(
                    agent.agent_id, physicsClientId=self.physics_client
                )
                self.camera_viewer["blue"].update_fixed_views(
                    pos=pos, team_name="blue", agent_idx=idx
                )
            ...
            for idx, agent in enumerate(self.red_team.agents):
                ...
                self.camera_viewer["red"].update_fixed_views(
                    pos=pos, team_name="red", agent_idx=idx
                )
