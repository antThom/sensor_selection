from panda3d.core import CompassEffect, PointLight, AmbientLight, LVector3, TextureStage, Loader
...
class SKY:
    ...
    def load_sun(self, config, loader, render, day_length, time_of_day):
        # Visible sun disk
        self.sun = loader.loadModel(config["light_source"].get("obj_path", 'models/misc/sphere'))
    ...
    def set_direction(self, day_length, time_of_day, azimuth0=0.0, R=5000):
        ...
        self.sun_light_np.setHpr(np.rad2deg(azimuth), np.rad2deg(inclination), 0)  # Heading, Pitch, Roll
        
        x, y, z = (R*np.cos(azimuth)*np.sin(inclination), R*np.sin(azimuth)*np.sin(inclination), R*np.sin(inclination))
        self.sun.setPos(x,y,z)

    def _update_sun(self, task_time):
        # Normalized time of day: 0..1
        u = ((self.start_time.hour*3600 + self.start_time.minute*60 + task_time) % self.day_length) / self.day_length
