...
class STATIC_OBJECT(OBJECT):
    ...
    def _set_texture(self,texture_path:str=None, scale:list=None):
        ...
        self.object.setTexGen(
            TextureStage.getDefault(),
            TexGenAttrib.MWorldPosition
        )

        if scale is not None:
            self.object.setTexScale(
                TextureStage.getDefault(),
                scale[0],
                scale[1]
            )
    ...
    def _set_position(self,pos:str=None, N:int=1, pos_val: np.ndarray= None, terrain=None, render=None):
        if pos =="center":
            # Get bounding box
            ...
            z = terrain.terrain_height_at(x=position[0], y=position[1], ray=ray, render=render, cTrav=self.cTrav, rayQueue=self.rayQueue)
            if z is None:
                pass
