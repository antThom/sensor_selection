...
class Environment:
    def __init__(self, client_id, config):
        ...
        self.thermal = ThermalManager(
            ambient_K=self.ambient_temp, T_sky=self.sky_temp, time_of_day=self.sim_time
        )
        ...
        terrain_file = os.path.join(
            base_dir, os.path.normpath(self.terrain_config["Layered Surface"]["Mesh"])
        )
        base_dir = os.getcwd()
    ...
    def load_terrain(self, config_file, base_dir):
        ...
        if "asc" in config_file:
            ...
            self.terrain["terrain_bounds"] = np.array(
                (
                    [
                        self.terrain["header"]["xllcorner"],
                        self.terrain["header"]["xllcorner"]
                        + self.terrain["header"]["cellsize"]
                        * self.terrain["header"]["ncols"],
                    ],
                    [
                        self.terrain["header"]["yllcorner"],
                        self.terrain["header"]["yllcorner"]
                        + self.terrain["header"]["cellsize"]
                        * self.terrain["header"]["nrows"],
                    ],
                )
            )
            self.terrain["terrain_area"] = np.product(
                self.terrain["terrain_bounds"][:, -1]
            )
        else:
        ...
        texture_file_name = os.path.join(
            base_dir,
            os.path.normpath(self.terrain_config["Layered Surface"]["Texture"]),
        )
    ...
    def load_features(self, base_dir):
        for idx, feat in enumerate(self.terrain_config["Surface Mesh"]):
            if idx > 0.0:
                mesh_file_name = (
                    str(feat["Mesh"]).strip().strip("'\"")
                )  # remove any stray quotes in the JSON
    ...
    def load_trees(self, feat, mesh_path):
        if feat["position_type"] == "patch":
            ...
            Num_feat = self.determine_N_features(
                feat=feat, id=tree_id, area=patch_polygon.area, N_min=5
            )
            # if "N" in feat:
            #     Num_feat = int(feat["N"])
            ...
            # else:
            #     Num_feat = 1
            tree_xy_positions = self.sample_points_in_polygon(
                patch_polygon, Num_feat, buffer=1.0
            )
        else:
            tree_xy_positions = np.random.uniform(low=-5, high=5, size=(1, 2))

        for x, y, z in tree_xy_positions:
            z_ter = -self.get_height_from_heightmap(
                x, y, self.terrain["height_data"], sx=10, sy=10, sz=1.0
            )
            tree_id = p.loadURDF(
                str(mesh_path),
                basePosition=[x, y, z],
    ...
    def load_clouds(self, feat, mesh_path):
        ...
        if feat["position_type"] == "patch" or "patch" in feat:
            ...
            Num_feat = self.determine_N_features(
                feat=feat, id=cloud_id, area=patch_polygon.area, N_min=1
            )
            ...
            Num_feat = self.determine_N_features(
                feat=feat, id=cloud_id, area=self.terrain["terrain_area"], N_min=5
            )
            # if "density" in feat:
            #     Num_feat = self.compute_N_from_density(id=cloud_id, density=feat['density'], N_min=5, area=self.terrain['terrain_area'])

            pos = np.random.uniform(
                low=-1 * np.max(self.terrain["terrain_bounds"]),
                high=1 * np.max(self.terrain["terrain_bounds"]),
                size=(Num_feat, 3),
            )
        if "altitude" in feat:
            pos[:, 2] = feat["altitude"]
        else:
            pos[:, 2] = np.random.uniform(low=20, high=100, size=(Num_feat, 1)).reshape(
                (-1,)
            )

        for x, y, z in pos:
            z_terrain = self.get_height_from_heightmap(
                x, y, self.terrain["height_data"], sx=10, sy=10, sz=1.0
            )
            cloud_id = p.loadURDF(
                str(mesh_path),
    ...
    def determine_N_features(self, feat, id, area=0, N_min=1):
        if "N" in feat:
            ...
            Num_feat = self.compute_N_from_density(
                id=id, density=feat["density"], N_min=N_min, area=area
            )
        else:
            Num_feat = 1
        return Num_feat
    ...
    def _asc_origin_from_header(self):
        ...
        if "xllcorner" in self.header and "yllcorner" in self.header:
            ...
            raise KeyError(
                "Header must contain xllcorner/yllcorner or xllcenter/yllcenter"
            )
        return x0, y0, cs

    def get_height_from_heightmap(
        self, x_world, y_world, heightmap, sx=0.1, sy=0.1, sz=1.0
    ):
