...
class ThermalManager:
    ...
    def add_object(
            ...
            material.update(
                {
                    ...
                    "beta": 1e-3 if beta is None else beta,
                    "gamma": 5e-10 if gamma is None else gamma,
                    "emiss": 0.95 if emiss is None else emiss,
                }
            )
        else:
        ...
        self.objects[(body_id, link_id)] = ThermalObject(
            ...
            init_T=init_T or material.get("T", self.ambient),
    ...
    def sun_dir(self):
        ...
        if hasattr(tod, "hour"):
            hour = tod.hour + tod.minute / 60.0 + tod.second / 3600.0
        elif isinstance(tod, (int, float)):
            parts = [float(x) for x in tod.split(":")]
            hour = (
                parts[0]
                + (parts[1] if len(parts) > 1 else 0) / 60.0
                + (parts[2] if len(parts) > 2 else 0) / 3600.0
            )
        else:
            hour = float(tod)
        elev = max(
            0.0, np.sin((hour - 6.0) / 12.0 * np.pi)
        )  # simple sine curve from 6am to 6pm
        horiz = np.sqrt(max(1.0 - elev**2, 0.0))
        azim = (hour / 24.0) * 2.0 * np.pi  # full rotation over 24 hours
        return [None, np.asarray([horiz * np.cos(azim), horiz * np.sin(azim), elev])][
            elev > 0.0
        ]
    ...
    def temp_map(self):
        ...
        return {
            k: (v.T if hasattr(v, "T") else v["T"]) for k, v in self.objects.items()
        }
    ...
    def update(self, dt, irradiance, wind=None, sun_dir=None):
        ...
        for k, obj in self.objects.items():
            if hasattr(obj, "get_temp"):
                ...
                dT = (
                    obj["alpha"] * irradiance
                    - obj["beta"] * (obj["T"] - self.ambient)
                    + radiative_term
                )
                obj["T"] += dT * dt
