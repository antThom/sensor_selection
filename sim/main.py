...
class PyBulletGoalEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.agent = p.loadURDF(
            "r2d2.urdf", basePosition=[0, 0, 0.1], physicsClientId=self.physics_client
        )
    ...
    def _get_obs(self):
        pos, _ = p.getBasePositionAndOrientation(
            self.agent, physicsClientId=self.physics_client
        )
        return np.array(
            [pos[0], pos[1], self.goal_pos[0], self.goal_pos[1]], dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        # Move agent
        p.resetBaseVelocity(
            self.agent, linearVelocity=[dx, dy, 0], physicsClientId=self.physics_client
        )
