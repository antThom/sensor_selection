...
class agent:
    def __init__(self, state, dim, dt, sensors):
        ...
        for sensor, params in sensors.items():
            if sensor == "bearing_sensor":
                self.sensors.append(
                    S.bearing_sensor(
                        num=params[0], FOV=params[1], R=params[2], cov=params[3]
                    )
                )
            elif sensor == "range_bearing_sensor":
                self.sensors.append(
                    S.range_bearing_sensor(
                        num=params[0], FOV=params[1], R=params[2], cov=params[3]
                    )
                )
            elif sensor == "range_bearing_range_rate_sensor":
                self.sensors.append(
                    S.range_bearing_range_rate_sensor(
                        num=params[0], FOV=params[1], R=params[2], cov=params[3]
                    )
                )
    ...
    def initialize_filter(self, dim, dt):
        F = np.vstack(
            [
                np.hstack([np.eye(dim), dt * np.eye(dim)]),
                np.hstack([np.zeros((dim, dim)), np.eye(dim)]),
            ]
        )
    ...
    def eom(self, target, env):
        if self.dim == 2:
            ...
            V = self.x[3]
            th = self.x[2]
            self.x += self.dt * np.vstack(
                [V * np.cos(th), V * np.sin(th), self.u[1], self.u[0]]
            )
