...
class bearing_sensor(SENSOR):
    ...
    def output(self, x_p, x_t, env):
        ...
        self.added_noise(
            agent_in_preclusion,
            targer_in_preclusion,
            np.linalg.norm(np.hstack([delta_y, delta_x])),
        )
    ...
    def added_noise(self, agent_in, target_in, R):
        ...
        for a_in, t_in in zip(agent_in, target_in):
            self.noise += np.random.normal(0, self.covariance) + (
                int(a_in[0]) * (np.random.normal(0, 2 * np.pi * a_in[1]))
                + int(t_in[0]) * (np.random.normal(0, 2 * np.pi * t_in[1]))
            )
...
class range_bearing_sensor(SENSOR):
    ...
    def output(self, x_p, x_t, env):
        ...
        self.y = np.vstack(
            [
                self.h[0, 0] + self.range_noise,
                (self.h[1, 0] + self.bearing_noise) % (2 * np.pi),
            ]
        ).reshape((2, 1))
        return self.y
    ...
    def added_noise(self, agent_in, target_in, R):
        ...
        for a_in, t_in in zip(agent_in, target_in):
            self.noise += (
                int(a_in[0]) * np.random.multivariate_normal(mean, covariance, 1)
            ) + (int(t_in[0]) * np.random.multivariate_normal(mean, covariance, 1))
...
class range_bearing_range_rate_sensor(SENSOR):
    ...
    def output(self, x_p, x_t, env):
        ...
        self.y = np.vstack(
            [
                self.h[0, 0] + self.range_noise,
                self.h[1, 0] + self.range_rate_noise,
                (self.h[2, 0] + self.bearing_noise) % (2 * np.pi),
            ]
        ).reshape((2, 1))
        return self.y
