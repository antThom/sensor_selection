...
def plot_scene(agents, targets, env, ax, fig):
    ...
    for agent in agents:
        ...
        # Plot the Sensor
        agent.sensors[agent.select].plot_sector(
            ax, (agent.x[0, 0], agent.x[1, 0]), agent.x[3, 0]
        )
    ...
    for ii in np.arange(env.num_preclusions):
        ...
        circle = plt.Circle(
            ...
            fill=True,
            linewidth=2,
            alpha=remap_noise(env.noise[ii]),
        )
        ax.add_patch(circle)
    ...
    ax.quiver(
        agents[0].xt_hat[0, 0],
        agents[0].xt_hat[1, 0],
        agents[0].xt_hat[2, 0] / speed_hat,
        agents[0].xt_hat[3, 0] / speed_hat,
        color=est_color,
        angles="uv",
    )
    ax.scatter(
        agents[0].xt_hat[0, 0], agents[0].xt_hat[1, 0], c=est_color, s=24, marker="+"
    )
